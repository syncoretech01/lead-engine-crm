...
Current implementation note (through Wave 1, Step 1.3A): initial actionable approvals, revision chains and replacement approvals, and final decisions now enqueue deterministic NotifyOutbox events inside their authoritative serializable transactions. Dashboard and chat use the same locked decision/revision services, and machine actors must be authorized members of the asserted workspace. A final NICHE_TEST validates the Contracts payload/hash and request/run/brief chain, approves the brief, creates one DRAFT Campaign, initializes RESEARCH as COMPLETED and HUB_SEARCH as PENDING, and enqueues one final event. Replay, concurrency, transaction retry, rollback, authentication failure, and seeded-route behavior are verified against PostgreSQL. Live Growth Bot and staging/production acceptance remain outstanding; see the implementation tracker for the exact next step.
