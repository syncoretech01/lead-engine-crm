...
import type { MyContactRow } from "@/components/crm/my-contacts-table";
...
/**
 * Side-peek body for an assigned contact (My Contacts). Fed entirely by the row
 * the table already has — no fetch. Mirrors ContactPeekContent but surfaces the
 * SDR-relevant fields (priority, SLA, assignment) and leads with a call action,
 * since My Contacts is the calling surface.
 */
export function MyContactPeekContent({
  contact,
  callerLabel,
  callBlockReason
}: {
  contact: MyContactRow;
  callerLabel?: string;
  callBlockReason?: string;
}) {
  const router = useRouter();
  const href = `/crm/contacts/${contact.contactId}`;
  ...
  return (
    <>
      <div className="border-b p-5">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <h2 className="truncate text-base font-semibold text-foreground">{contact.contactName}</h2>
            {contact.title ? (
      ...
      <div className="flex-1 divide-y px-5">
        ...
        <Field label="SLA">
          <StatusBadge label={contact.slaStatus} tone={contact.slaTone} />
        </Field>
        <Field label="Assigned">{contact.assignedRelative || contact.assignedDate || "—"}</Field>
        <Field label="Last touch">{contact.lastTouchLabel || "—"}</Field>
        <Field label="Owner">{contact.ownerName || "—"}</Field>
      </div>
